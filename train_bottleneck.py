...
ANNOTATION_PATH = 'master_labels.txt'
LOG_DIR = 'logs/000/'
CLASSES_PATH = 'model_data/rs_classes.txt'
ANCHORS_PATH = 'model_data/tiny_yolo_anchors.txt'
WEIGHTS_PATH = 'model_data/tiny_yolo_weights.h5'
BOTTLENECK_FILENAME = "bottlenecks.npz"
YOLO_TINY =True
...
def _main():
    ...
    if YOLO_TINY:
        model, bottleneck_model, last_layer_model = create_tiny_bottleneck_model(input_shape, anchors, num_classes,
                         freeze_body=2, weights_path=WEIGHTS_PATH)  # make sure you know what you freeze
    else:
        model, bottleneck_model, last_layer_model = create_bottleneck_model(input_shape, anchors, num_classes,
                        freeze_body=2, weights_path=WEIGHTS_PATH) # make sure you know what you freeze
    ...
    if True:
        ...
        if not os.path.isfile(BOTTLENECK_FILENAME):
            ...
            if YOLO_TINY:
                np.savez(BOTTLENECK_FILENAME, bot0=bottlenecks[0], bot1=bottlenecks[1])
            else:
                np.savez(BOTTLENECK_FILENAME, bot0=bottlenecks[0], bot1=bottlenecks[1], bot2=bottlenecks[2])

        # load bottleneck features from file
        dict_bot=np.load(BOTTLENECK_FILENAME)
        if YOLO_TINY:
            bottlenecks_train = [dict_bot["bot0"][:num_train], dict_bot["bot1"][:num_train]]
            bottlenecks_val = [dict_bot["bot0"][num_train:], dict_bot["bot1"][num_train:]]
        else:
...
def create_bottleneck_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
...
def create_tiny_bottleneck_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
            weights_path='model_data/tiny_yolo_weights.h5'):
    '''
    create the training model, for Tiny YOLOv3
    N.B. This is a Markham made function
    '''
    K.clear_session() # get a new session
    image_input = Input(shape=(None, None, 3))
    h, w = input_shape
    ...
    # get output of second last layers and create bottleneck model of it
    out1 = model_body.layers[40].output
    out2 = model_body.layers[41].output
    bottleneck_model = Model([model_body.input, *y_true], [out1, out2])

    # create last layer model of last layers from yolo model
    in0 = Input(shape=bottleneck_model.output[0].shape[1:].as_list())
    in1 = Input(shape=bottleneck_model.output[1].shape[1:].as_list())
    last_out0 = model_body.layers[42](in0)
    last_out1 = model_body.layers[43](in1)
    model_last = Model(inputs=[in0, in1], outputs=[last_out0, last_out1])
    model_loss_last = Lambda(yolo_loss, output_shape=(1,), name='yolo_loss',
                             arguments={'anchors': anchors, 'num_classes': num_classes, 'ignore_thresh': 0.5})(
        [*model_last.output, *y_true])
    last_layer_model = Model([in0, in1, *y_true], model_loss_last)
...
def bottleneck_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, bottlenecks):
    ...
    while True:
        ...
        y_true = preprocess_true_boxes(box_data, input_shape, anchors, num_classes)
        if YOLO_TINY:
            yield [b0, b1, *y_true], np.zeros(batch_size)
        else:
